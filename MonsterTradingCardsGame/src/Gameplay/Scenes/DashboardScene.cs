...
public class DashboardScene : Scene
{
    ...
    private void DrawStatBar()
    {
        ...
        Gui.DrawAt(MonsterAssets.Goblin, new Coord { X = 83, Y = 1 });

        Gui.DrawAt(CardAssets.CardBody, new Coord { X = 2, Y = 15 });
        Gui.DrawAt(MonsterAssets.Elf, new Coord { X = 3, Y = 16 });
        Gui.DrawAt(CardAssets.CardBody, new Coord { X = 22, Y = 15 });
        Gui.DrawAt(MonsterAssets.Knight, new Coord { X = 23, Y = 16 });
        Gui.DrawAt(CardAssets.CardBody, new Coord { X = 42, Y = 15 });
        Gui.DrawAt(MonsterAssets.Dragon, new Coord { X = 43, Y = 16 });
        Gui.DrawAt(CardAssets.CardBody, new Coord { X = 62, Y = 15 });
        Gui.DrawAt(MonsterAssets.Kraken, new Coord { X = 63, Y = 16 });
        Gui.DrawAt(CardAssets.CardBody, new Coord { X = 82, Y = 15 });
        Gui.DrawAt(MonsterAssets.Goblin, new Coord { X = 83, Y = 16 });
    }
}